...
export default function CurrentRoom() {
  ...
  const handleScanSuccess = async (decodedText, decodedResult) => {
    ...
    const scannedRoomId = decodedText.trim().replace(/^Room_/, '');
    const expectedRoomId = booking?.room_id?.toString().trim(); 
  
    console.log("So sánh QR:", scannedRoomId, "==", expectedRoomId);
  
    if (!expectedRoomId || scannedRoomId !== expectedRoomId) {
      toast.error("Mã QR không khớp với phòng bạn đã đặt!");
      return;
    }
  
    try {
      await handleCheckInOut(); 
      toast.success(`Quét QR thành công! Đã ${isCheckedIn ? 'check-out' : 'check-in'}`);
    } catch (error) {
      console.error("Error processing QR code:", error);
      toast.error("Có lỗi khi xử lý. Hãy thử lại hoặc dùng nút check-in/out.");
    }
  };
  ...
  useEffect(() => {
    if (isScanning) {
      try {
        ...
        scanner.render(handleScanSuccess, (err) => console.error(err));
        scannerRef.current = scanner;
      } catch (error) {
        console.error("Failed to initialize QR scanner:", error);
        toast.error("Không thể khởi tạo máy quét QR. Vui lòng thử lại hoặc sử dụng nút Check In/Out.");
        setIsScanning(false);
      }
    }
        ...
        <div className="bg-white bg-opacity-40 shadow-lg rounded-lg flex w-3/4 gap-x-4 h-[80%] max-h-[600px]">
          ...
          <div className="w-1/3 flex justify-center items-center">
            <div className="bg-gray-300 flex flex-col justify-between items-center h-[90%] w-[90%] bg-opacity-50 py-8 rounded-lg">
              ...
              <i
                className="fas fa-camera text-9xl text-gray-700 cursor-pointer"
